...
import me.TristanPopken.OpenGL.GameEngine.Meshes.textures.TextureData;
...
class VBO {
	...
	public VBO(int vaoID, int vboID) {
		this.vaoID = vaoID;
		this.vboID = vboID;
	}
...
public class Loader {
	...
	public void unloadVao(int VaoID) {
		
		for (int i = 0; i < vbos.size(); i++) {
			VBO vbo = vbos.get(i);
			int vaoID = vbo.getVaoID();
			int vboID = vbo.getVboID();
			if (vaoID == VaoID) {
				//System.out.println("Deleting VBO "+vboID+" in VAO "+vaoID);
...
//		GL15.glBindBuffer(GL15.GL_ARRAY_BUFFER, 0);
	...
	public int[] loadTextures(String[] fileNames) {
		int[] textureIDs = new int[fileNames.length];
	...
	public void cleanUP() {
		...
		for (VBO vbo : vbos) {
			GL15.glDeleteBuffers(vbo.getVboID());
		}
	...
	private void storeDataInAttributeList(int attributeNumber, int coordinateSize, float[] data) {
		int vboID = GL15.glGenBuffers();
		vbos.add(new VBO(activeVAO, vboID));
		GL15.glBindBuffer(GL15.GL_ARRAY_BUFFER, vboID);
		FloatBuffer buffer = storeDataInFloatBuffer(data);
		GL15.glBufferData(GL15.GL_ARRAY_BUFFER, buffer, GL15.GL_STATIC_DRAW);
		GL20.glVertexAttribPointer(attributeNumber, coordinateSize, GL11.GL_FLOAT, false, 0, 0);
